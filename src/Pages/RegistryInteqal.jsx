...
import { getRegistryInteqalFiles } from "../api/RegistryInteqal";
import { useTheme } from '@emotion/react';
import { useNavigate } from "react-router-dom";
import DataTable from "../Components/Table";


function RegistryInteqal() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const responseData = await getRegistryInteqalFiles(); // Call the function to get the data
  ...
  return (
    ...
    >
        <Typography variant="h4" sx={{ flexGrow: 1 , mb:'1%' }}>
             Registry / Inteqal Files
        </Typography>
        </Box>
        <Button  sx={ButtonStyling}
                onClick={() => navigate('/Create/RegistryInteqal')}
            >
            Create Registry/Inteqal <CreateIcon sx={{ml:1}} />
        </Button>
...
export default RegistryInteqal;
