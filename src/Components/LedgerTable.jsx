...
export default function LedgerTable({ data, nav, isPayment }) {
  ...
  const handleAction1 = (id) => {
    const selectedRowData = data.find((row) => row.Application_No === id);

    console.log(selectedRowData)
    navigate(`/${nav}/${id}` , { state: { ledger: selectedRowData } });
  };
