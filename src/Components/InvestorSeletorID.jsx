...
export default function InvestorSelector({ onConfirm }) {
  ...
  useEffect(() => {
    const fetchAgents = async () => {
      try {
        const responseData = await getAllInvestors();
  ...
  const VirtualizedList = React.forwardRef(({ children, ...rest }, ref) => (
    ...
    >
      {({ index, style }) => (
        <MenuItem
          key={agents[index].RegisterationID}
          value={agents[index].RegisterationID}
          style={style}
          onClick={() => handleChange({ target: { value: agents[index].RegisterationID } })}
        >
          {agents[index].Name}
        </MenuItem>
      )}
  ...
  return (
    <div>
      <FormControl sx={{  width: 270 }}>
        <InputLabel id="demo-multiple-chip-label">Investor</InputLabel>
        <Select
